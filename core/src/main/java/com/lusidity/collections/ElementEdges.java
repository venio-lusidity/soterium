...
public class ElementEdges<T extends DataVertex> extends ElementAttributes<T>
{
	private boolean loaded=false;

	// Constructors
	public ElementEdges(PropertyAttributes propertyAttributes)
	{
	...
	/**
	 * Save the main vertex.  This creates a choke point but maybe necessary because of some multi-threading conditions.
	 * This shouldn't really ever get called.
	 * @param vertex  The vertex to save.
	 */
	public static synchronized void saveVertex(DataVertex vertex)
	{
		if (!vertex.hasId())
		{
			try
	...
	public T getAt(int idx)
	{
		...
		if (idx<size)
		{
			ElementIterator iterator=new ElementIterator(this.getVertex(), this.getQuery(0, 0), 0, idx, 1);
	...
	public boolean contains(Object o)
	{
		...
		if (this.getVertex().hasId())
		{
			...
			if (ClassX.isKindOf(o.getClass(), this.getExpectedType()))
			{
				...
				{
					Edge edge=this.getVertex().getEdgeHelper().getEdge(this.getEdgeType(), fVertex, this.getKey(), this.getDirection());
					result=(null!=edge);
				}
			}
		}
