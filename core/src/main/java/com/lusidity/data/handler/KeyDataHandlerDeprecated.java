...
public class KeyDataHandlerDeprecated implements IKeyDataHandler
{
	// Overrides
	@Override
	public KeyDataTransformed handleSetterBefore(Object newValue, KeyData keyData)
	{
		if(keyData.getVertex().hasId())
		{
			keyData.getVertex().setDeprecatedChanged(true);
		}
	...
	{
		if(keyData.getVertex().fetchDeprecated().getValue()){
			Class<? extends DataVertex> cls = keyData.getVertex().getClass();
			try
			{
				Environment.getInstance().getCache().remove(cls, cls, keyData.getVertex());
			}
