...
public class KeyDataHandlerTitleEmpty implements IKeyDataHandler
{
	// Overrides
	@Override
	public KeyDataTransformed handleSetterBefore(Object newValue, KeyData keyData)
	{
		// If the title is null or empty and the newValue is not use the newValue as the title.
		if((newValue instanceof String) && ClassX.isKindOf(keyData.getVertex(), ApolloVertex.class)){
			ApolloVertex apolloVertex = (ApolloVertex) keyData.getVertex();
			if(apolloVertex.fetchTitle().isNullOrEmpty()){
				apolloVertex.fetchTitle().setValue((String) newValue);
			}
		}
