        ...
        methods = {
            ...
            header: function() {
                function make() {
                    ...
                    function count(filtered, e, s){
                        var c;
                        if(!filtered) {
                            c = dCrt('span').html(state.paging.count + " of " + state.paging.hits +
                            ...
                            c = dCrt('span').html(String.format('{0} of {1} filtered', s, state.paging.hits));
                        }

                        if(!state.opts.realTime && state.asOf) {
                            c.append('as of ').append(state.asOf);
                        }
                        return dCrt('div').append(c).css({fontSize: '10px'});
                    }
                    var title = dCrt('div').append(state.opts.title);

                    state.table.on('table-view-filtered', function (e) {
                        var s = (e.item) ? e.item.length : 0;
                        var c = count((s<state.paging.hits), e, s);
                        state.header.head.children()[0].remove();
                        state.header.head.prepend(c);
                    });
                ...
                details:{
                    init: function (data, container, scrlTop) {
                        try {
                            ...
                            opt.data = data;
                            container.append(state.table);
                            state.table.on("tableViewDataLoaded", function () {
                                methods.adjustTable();
                            });
                            state.table.pTableView(opt);
                            state.table.on('table-view-filtered', function () {
                               state.trigger('table-view-filtered');
                            });
                            state.table.on('table-view-created', function () {
