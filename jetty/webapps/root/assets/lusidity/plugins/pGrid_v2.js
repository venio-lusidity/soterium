                ...
                if ($.jCommon.json.getLength(state.sort) > 0) {
                    if(qry.sorted){
                        ...
                        $.each(state.sort, function () {
                           qry.sorted.push({key: this.property, asc: this.asc});
                        });
                    }
                    else {
                        qry.sort = state.sort;
                ...
                init: function () {
                    ...
                    $.each(state.opts.mapping, function () {
                        ...
                        if (map.sortable) {
                            ...
                            n.on('click', function () {
                                toggle($(this));
                                if($.isFunction(map.onBeforeSort)){
                                    map.onBeforeSort(map);
                                }
                                methods.sort(map);
                            });
                        }
                ...
                init: function () {
                    ...
                    if (state.opts.getQuery) {
                        if (state.page.data.length > 0) {
                            if(!state.opts.paging.enabled && $.isFunction(state.opts.onBefore)) {
                            ...
                            function callback(data, requestOn) {
                                ...
                                methods.content.make();
                                if(nd){
                                    methods.content.noData(state.tableNode);
                                }
                            }
        ...
        if (typeof method === "object") {
            ...
            switch (method) {
                ...
                case 'setContentHeight':
                    $pGrid.setContentHeight(options);
                    break;
                case 'state':
                default:
                    return $pGrid;
