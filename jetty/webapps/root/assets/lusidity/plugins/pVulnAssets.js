...
;(function ($) {
    ...
    //Object Instance
    $.pVulnAssets = function(el, options) {
        var state = el,
            methods = {};
        state.container = $(el);

        state.worker = (options.schema && options.schema.plugin) ? options : {node: state, data: options.data };
        state.opts = $.extend({}, $.pVulnAssets.defaults, (options.schema && options.schema.plugin) ? options.schema.plugin : options);
        ...
        var opts = {
            ...
            getQuery: function(){
                ...
                if(data.vulnId) {
                    var frmt = String.format('"{0}.folded":"{1}"', "vulnId", data.vulnId.toLowerCase());
                    var match = '{"match":{' + frmt + '}}';
                    must.push(JSON.parse(match));
                }
                else{
                    var frmt = String.format('"{0}.raw":"{1}"', "relatedId", data[state.KEY_ID]);
                    var match = '{"match":{' + frmt + '}}';
                    must.push(JSON.parse(match));
                }
    ...
    //Plugin Function
    $.fn.pVulnAssets = function(method, options) {
        if (method === undefined) method = {};

        if (typeof method === "object") {
            return new $.pVulnAssets($(this),method);
        } else {
            // Helper strings to quickly perform functions
            var $pVulnAssets = $(this).data('pVulnAssets');
            switch (method) {
                case 'exists': return (null!==$pVulnAssets && undefined!==$pVulnAssets && $pVulnAssets.length>0);
                case 'reset': $pVulnAssets.reset(options);break;
                case 'resize': $pVulnAssets.resize(options);break;
                case 'state':
                default: return $pVulnAssets;
            }
        }
    };

    $.pVulnAssets.call= function(elem, options){
        elem.pVulnAssets(options);
    };

    try {
        $.htmlEngine.plugins.register("pVulnAssets", $.pVulnAssets.call);
    }
    catch(e){
        console.log(e);
    }
