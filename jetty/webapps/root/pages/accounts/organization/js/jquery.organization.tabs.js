        ...
        methods = {
            ...
            pnlMiddle: {
                ...
                resize: function () {
                    ...
                    if(state.opts.pNode){
                        var hh = (state.opts.hdrNode.height()-28);
                        dMax(state.opts.pNode, h-hh);
                        dMax(state.opts.scNode, h-hh);
                    }
                },
                ...
                content: {
                    init: function(){
                        ...
                        var tabs = dCrt('ul').addClass('nav nav-tabs');
                        var tabContent = dCrt('div').addClass('tab-content');
                        content.append(tabs).append(tabContent);
                        
                        var tsc = dCrt('li').addClass(state.tabLast===0 ? 'active' : '');
                        var tscl = dCrt('a').addClass('no-radius').attr('href', "#tsc").attr('data-toggle', 'tab').html(state.pts.pos.title);
                        tscl.on('click', function () {
                           state.tabLast = 0;
                        });
                        tscl.attr('title', state.pts.pos.desc);
                        tsc.append(tscl);
                        tabs.append(tsc);

                        var tscp = dCrt('li').addClass(state.tabLast===1 ? 'active' : '');
                        var tscpl = dCrt('a').addClass('no-radius').attr('href', "#tscp").attr('data-toggle', 'tab').html(state.pts.sc_pos.title);
                        tscpl.on('click', function () {
                            state.tabLast = 1;
                        });
                        tscpl.attr('title', state.pts.sc_pos.desc);
                        tscp.append(tscpl);
                        tabs.append(tscp);

                        state.opts.pNode = dCrt('div').addClass('tab-pane ' + (state.tabLast===0 ? 'active' : '')).attr('id', 'tsc').css({overflowX: 'none', overflowY: 'auto'});
                        state.opts.scNode = dCrt('div').addClass('tab-pane ' + (state.tabLast===1 ? 'active' : '')).attr('id', 'tscp').css({overflowX: 'none', overflowY: 'auto'});
                        tabContent.append(state.opts.pNode).append(state.opts.scNode);

                        methods.pnlMiddle.content.make(state.opts.pNode, state.pts.pos, state.current.item, 'hdr-pos');
                        methods.pnlMiddle.content.make(state.opts.scNode, $.extend({}, state.pts.sc_pos), state.current.item, 'hdr-sc-pos');
                        methods.pnlMiddle.resize();
                    },
                    makeTab: function(){
                        var container = dCrt('div');
                        ...
                        var pnlBdy = dCrt('div');
                        container.append(pnlBdy);
                        /*
                        $.htmlEngine.panel(container,pt.glyph,pt.title,null,false,null,(pt.menu ? pt.menu.opts : null));
                    if(cls){
                        $(container.panel('getHeader')).addClass(cls);
                    }
                    if(pt.desc){
                        $(container.panel('getHeader')
                            .find('.pnl-title')).attr('data-toggle', 'tooltip').attr('data-placement', 'bottom').attr('title', pt.desc).tooltip().addClass('header-text');
                    ...
                    */
                    ...
                    make: function(parentNode, pt, item, cls){
                        var container = dCrt('div');
                        parentNode.append(container);
                        if(pt.menu && pt.menu.opts){
                            pt.menu.opts.target = container;
                    ...
                    var s = function (data) {
                        if(data && data.results){
                            ...
                            $.each(ps, function () {
                                ...
                                header.append(link);
                                var ul = dCrt('ul').css({marginLeft: '25px'});
                                p.append(ul);
                                methods.draggable(p, item);
                                state.opts.positionsContNode.append(p);
                                var sc = function (useritem) {
                                    if(useritem && useritem.results && useritem.results.length>0){
                                        var useritems = $.jCommon.array.sort(useritem.results, [{property: 'title', asc: true}]);
                                        $.each(useritems, function () {
                                            var li = dCrt('li');
                                            var lk = dLink(this.title, this[state.KEY_ID]);
                                            li.append(lk);
                                            ul.append(li);
                                        });
                                    }
                                ...
                                $.htmlEngine.request('/query?limit=10000000', sc, sc, q, 'post');
                            });
                        }
                    };
                    var q = {
