;(function ($) {
        ...
        methods = {
            ...
            transform: function () {
                ...
                $.each(d.results, function () {
                    var id = this.otherId;
                    this.vulnId = $.jCommon.string.replaceAll(this.vulnId, "_rule", "").toUpperCase();
                    var a = map[id];
                    if(!a){
                        map[id] = a = this;
                        a._subs = [];
            ...
            html:{
                ...
                filter: function (fltr, fltr2, mc, rc, item) {
                    ...
                    if($.jCommon.string.equals(fltr, '***')){
                        ...
                        data = methods.html.applyFilter(fltr, fltr2, true);
                        if(rc) {
                            methods.html.subChart(fltr, mc, rc, data);
                        }
                    }
                    data.results = $.jCommon.array.sort(data.results, [{property: 'packedVulnerabilityMatrix', asc: false}]);
                ...
                applyFilter: function (fltr, fltr2, unique) {
                    ...
                    $.each(state.opts.data._subs, function (k, v) {
                        var sub = v;
                        if ($.jCommon.string.equals(sub.severity, fltr, true)) {
                            if (fltr2) {
                                if ($.jCommon.string.equals(sub.vulnId, fltr2, true)) {
                                    ids.push(k);
                                }
                            }
                            else {
                ...
                chart: function (node, data) {
                    ...
                    $.each(data._subs, function (k,v) {
                        var item = v;
                        var lid = k.toString();
                        item._color1 = rColors(item.severity);
                        item._hvrColor1 = $.jCommon.colors.lighten(item._color1, 10);
                        var key = $.jCommon.string.makeKey(item.severity);
                        var val = working[key];
                        if(!val){
                            val={
                                label: item.severity,
                                folded: item.severity.toLowerCase(),
                                value: 0,
                                count: 0,
                                ...
                                legend: {
                                    onMake: function (mc, rc, item) {
                                        ...
                                        var d = dCrt('span').addClass('active-link').attr('title', 'click once to apply filter click again to remove filter')
                                            .html(String.format('{0} {1}({2}): {3}', item.perc, item.enumerated, item.value, item.label))
                                        ...
                                        d.on('click', function () {
                                            if(d.hasClass('active')){
                                                ...
                                                d.addClass('active');
                                                state.opts.legend = item.legend.data.severity;
                                                methods.html.filter(item.legend.data.severity, null, mc, rc, item);
                                            }
                                        });
                            ...
                            working[key] = val;
                            var data = methods.html.applyFilter(item.severity, null, true);
                            val.value = data.hits;
                    ...
                    var d = dCrt('div').css({'position': 'relative', top: '10px'}).html(String.format("Findings Report: {0} ({1}) Assets ({2}) Categories", title, data.results.length, len));
                    node.jChartIt({sorted: true, max: state.opts.current.max, glyph: 'glyphicons-stats', title: d, dividedBy: db, width: 350, height: 175, onClick: onClick, data: options, type: 'bar', legendOverride: true, legend: String.format('Severity Summation by Assets: {0} Categories', len)});
                },
                subChart: function (fltr, mc, rc, data) {
                    ...
                    $.each(data._subs, function (k,v) {
                        var item = v;
                        if(!$.jCommon.string.equals(fltr, item.severity, true)){
                            return true;
                        }
                        subs.push(item);
                        var lid = k.toString();
                        ...
                        var key = $.jCommon.string.makeKey(item.vulnId);
                        var val = working[key];
                        if(!val){
                            val={
                                label: item.vulnId,
                                folded: item.vulnId.toLowerCase(),
                                value: 0,
                                color: item._color2,
                                ...
                                legend: {
                                    onMake: function (mc, rc, item) {
                                        ...
                                        var mb = dCrt('div').addClass('media-body').css({fontSize: '12px'});
                                        var n = dCrt('div');
                                        var d = dCrt('span').addClass('active-link').attr('title', 'click once to apply filter click again to remove filter').html(String.format('{0} ({1}): {2}', item.perc, item.value, item.legend.data.vulnId)).attr('title', item.legend.data.vuln);
                                        ...
                                        d.on('click', function () {
                                            if(d.hasClass('active')){
                                                ...
                                                mc.find('.media-body .active').removeClass('active');
                                                d.addClass('active');
                                                methods.html.filter(state.opts.legend, item.legend.data.vulnId, mc, rc, item);
                                            }
                                        });
                    ...
                    var lgnd = dCrt('span');
                    var s1 = dCrt('span').html('Findings:');
                    var s2 = dCrt('span').html(t).attr('title', 'Enumerated Findings');
                    var s3 = dCrt('span').html(String.format('[{0}]', options.length)).attr('title', 'Unique Findings');
    ...
    //Plugin Function
    $.fn.adHocScp = function(method, options) {
        if (method === undefined) method = {};

        if (typeof method === "object") {
            return this.each(function() {
                new $.adHocScp($(this), method);
            });
        } else {
            // Helper strings to quickly perform functions
            var $adHocScp = $(this).data('adHocScp');
            switch (method) {
                case 'exists': return (null!==$adHocScp && undefined!==$adHocScp && $adHocScp.length>0);
                case 'state':
                default: return $adHocScp;
            }
        }
    };
