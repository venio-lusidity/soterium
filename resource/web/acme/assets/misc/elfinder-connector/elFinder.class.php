...
class elFinder {
	...
	protected function get($args) {
		...
		if (($content = $volume->getContents($target)) === false) {
			return array('error' => $this->error(self::ERROR_OPEN, $volume->file($target), $volume->error()));
		}
		
		$json = json_encode($content);

		if ($json == 'null' && strlen($json) < strlen($content)) {
			return array('error' => $this->error(self::ERROR_NOT_UTF8_CONTENT, $volume->file($target)));
		}
	...
	protected function put($args) {
		...
		if (($file = $volume->putContents($target, $args['content'])) == false) {
			return array('error' => $this->error(self::ERROR_SAVE, $volume->file($target), $volume->error()));
		}
	...
	protected function extract($args) {
		...
		return ($file = $volume->extract($target))
			? array('added' => array($file))
			: array('error' => $this->error(self::ERROR_EXTRACT, $volume->file($target), $volume->error()));
	}
	...
	protected function resize($args) {
		...
		return ($file = $volume->resize($target, $width, $height, $x, $y, $mode, $bg, $degree))
			...
			: array('error' => $this->error(self::ERROR_RESIZE, $volume->file($target), $volume->error()));
	}
