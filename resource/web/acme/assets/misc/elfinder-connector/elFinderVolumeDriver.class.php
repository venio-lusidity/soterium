...
abstract class elFinderVolumeDriver {
	...
	protected $options = array(
		...
		// root directory file
		'file'            => '',
		// open this file on initial request instead of root file
		'startPath'       => '',
		...
		'mimeDetect'      => 'auto',
		// mime.types file file (for mimeDetect==internal)
		'mimefile'        => '',
	 ...
	 * By default set thumbnails file and image manipulation library.
	 *
	 * @return void
	 * @author Dmitry (dio) Levashov
	 **/
	protected function configure() {
		// set thumbnails file
		$file = $this->options['tmbPath'];
		if ($file) {
			if (!file_exists($file)) {
				if (@mkdir($file)) {
					chmod($file, $this->options['tmbPathMode']);
				} else {
					$file = '';
				}
			} 
			...
			if (is_dir($file) && is_readable($file)) {
				$this->tmbPath = $file;
				$this->tmbPathWritable = is_writable($file);
			}
		}
	...
	public function mount(array $opts) {
		...
		$this->root = $this->_normpath($this->options['file']);
		...
		if ($root['read']) {
			// check startPath - file to open by default instead of root
	...
	public function options($hash) {
		return array(
			'file'          => $this->_path($this->decode($hash)),
			'url'           => $this->URL,
			'tmbUrl'        => $this->tmbURL,
	...
	/**
	 * Return file file related to root
	 *
	...
	/**
	 * Return file real file if file exists
	 *
	 * @param  string  $hash  file hash
	 * @return string
	 * @author Dmitry (dio) Levashov
	...
	public function realpath($hash) {
		...
		return $this->stat($file) ? $file : false;
	}
	...
	public function closest($hash, $attr, $val) {
		return ($file = $this->closestByAttr($this->decode($hash), $attr, $val)) ? $this->encode($file) : false;
	}
	...
	public function file($hash) {
		...
		return ($file = $this->stat($file)) ? $file : $this->setError(elFinder::ERROR_FILE_NOT_FOUND);
	...
	public function ls($hash) {
		...
		foreach ($this->getScandir($file) as $stat) {
	...
	public function tree($hash='', $deep=0, $exclude='') {
		$file = $hash ? $this->decode($hash) : $this->root;
		
		if (($dir = $this->stat($file)) == false || $dir['mime'] != 'directory') {
			return false;
		}
		
		$dirs = $this->gettree($file, $deep > 0 ? $deep -1 : $this->treeDeep-1, $this->decode($exclude));
	...
	public function parents($hash) {
		...
		while ($file && $file != $this->root) {
			...
			if ($file != $this->root) {
				foreach ($this->gettree($file, 0) as $dir) {
	...
	public function tmb($hash) {
		...
		if (isset($stat['tmb'])) {
			return $stat['tmb'] == "1" ? $this->createTmb($file, $stat) : $stat['tmb'];
		}
		return false;
	}
	...
	public function mkdir($dst, $name) {
		...
		$file = $this->decode($dst);
		$dst  = $this->_joinPath($file, $name);
		$stat = $this->stat($dst);
		if (!empty($stat)) {
			return $this->setError(elFinder::ERROR_EXISTS, $name);
		}
		$this->clearcache();
		return ($file = $this->_mkdir($file, $name)) ? $this->stat($file) : false;
	}
	...
	public function mkfile($dst, $name) {
		...
		if ($this->stat($this->_joinPath($file, $name))) {
			return $this->setError(elFinder::ERROR_EXISTS, $name);
		}
		$this->clearcache();
		return ($file = $this->_mkfile($file, $name)) ? $this->stat($file) : false;
	}
	...
	public function duplicate($hash, $suffix='copy') {
		...
		return ($file = $this->copy($file, $dir, $this->uniqueName($dir, $this->_basename($file), ' '.$suffix.' '))) == false
			? false
			: $this->stat($file);
	}
	...
	public function upload($fp, $dst, $name, $tmpname) {
		...
		// $this->clearcache();
		if (($file = $this->_save($fp, $dstpath, $name, $mime, $w, $h)) == false) {
			return false;
		}
	...
	public function paste($volume, $src, $dst, $rmSrc = false) {
		...
		if (($test = $volume->closest($src, $rmSrc ? 'locked' : 'read', $rmSrc))) {
			return $rmSrc
				? $this->setError($err, $errpath, elFinder::ERROR_LOCKED, $volume->file($test))
		...
		if ($volume == $this) {
			...
			if ($this->_inpath($destination, $source)) {
				return $this->setError(elFinder::ERROR_COPY_INTO_ITSELF, $file);
			}
			$method = $rmSrc ? 'move' : 'copy';
			
			return ($file = $this->$method($source, $destination, $name)) ? $this->stat($file) : false;
		}
		...
		if (($file = $this->copyFrom($volume, $src, $destination, $name)) == false) {
			return false;
		}
	...
	public function putContents($hash, $content) {
		...
		$this->clearcache();
		return $this->_filePutContents($file, $content) ? $this->stat($file) : false;
	}
	...
	public function extract($hash) {
		...
		$file   = $this->decode($hash);
		$parent = $this->stat($this->_dirname($file));

		if (!$file['read'] || !$parent['write']) {
			return $this->setError(elFinder::ERROR_PERM_DENIED);
		}
		$this->clearcache();
		return ($file = $this->_extract($file, $archiver)) ? $this->stat($file) : false;
	}
	...
	public function archive($hashes, $mime) {
		...
		foreach ($hashes as $hash) {
			...
			$files[] = $this->_basename($file);
		}
		
		$name = (count($files) == 1 ? $files[0] : 'Archive').'.'.$archiver['ext'];
		$name = $this->uniqueName($dir, $name, '');
		$this->clearcache();
		return ($file = $this->_archive($dir, $files, $name, $archiver)) ? $this->stat($file) : false;
	}
	...
	public function resize($hash, $width, $height, $x, $y, $mode = 'resize', $bg = '', $degree = 0) {
		...
		switch($mode) {
			...
			case 'propresize':
				$result = $this->imgResize($file, $width, $height, true, true);
				break;

			case 'crop':
				$result = $this->imgCrop($file, $width, $height, $x, $y);
				break;

			case 'fitsquare':
				$result = $this->imgSquareFit($file, $width, $height, 'center', 'middle', ($bg ? $bg : $this->options['tmbBgColor']));
				break;

			case 'rotate':
				$result = $this->imgRotate($file, $degree, ($bg ? $bg : $this->options['tmbBgColor']));
				break;

			default:
				$result = $this->imgResize($file, $width, $height, false, true);
				break;
		}
	...
	protected function encode($file) {
		if ($file !== '') {
			...
			// cut ROOT from $file for security reason, even if hacker decodes the file he will not know the root
			$p = $this->_relpath($file);
			// if reqesting root dir $file will be empty, then assign '/' as we cannot leave it blank for crypt
			if ($p === '')	{
				$p = DIRECTORY_SEPARATOR;
			}

			// TODO crypt file and return hash
			$hash = $this->crypt($p);
	...
	protected function decode($hash) {
		if (strpos($hash, $this->id) === 0) {
			...
			// TODO uncrypt hash and return file
			$file = $this->uncrypt($h);
			// append ROOT to file after it was cut in encode
			return $this->_abspath($file);//$this->root.($file == DIRECTORY_SEPARATOR ? '' : DIRECTORY_SEPARATOR.$file);
		}
	}
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function crypt($file) {
		return $file;
	}
	
	/**
	 * Return uncrypted file
	 * Not implemented
	 *
	 * @param  mixed  hash
	 * @return mixed
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function attr($file, $name, $val=false) {
		...
		if ($this->access) {
			if (is_array($this->access)) {
				...
				$perm   = $obj->{$method}($name, $file, $this->options['accessControlData'], $this);
			} else {
				$func = $this->access;
				$perm = $func($name, $file, $this->options['accessControlData'], $this);
			}
		...
		for ($i = 0, $c = count($this->attributes); $i < $c; $i++) {
			...
			$p = $this->separator.$this->_relpath($file);
	 ...
	 * Return fileinfo 
	 *
	 * @param  string  $file  file cache
	 * @return array
	 * @author Dmitry (dio) Levashov
	 **/
	protected function stat($file) {
		return isset($this->cache[$file])
			? $this->cache[$file]
			: $this->updateCache($file, $this->_stat($file));
	}
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function updateCache($file, $stat) {
		if (empty($stat) || !is_array($stat)) {
			return $this->cache[$file] = array();
		}

		$stat['hash'] = $this->encode($file);
		...
		if ($root) {
			...
			if (empty($stat['name'])) {
				$stat['name'] = $this->_basename($file);
			}
			if (empty($stat['phash'])) {
				$stat['phash'] = $this->encode($this->_dirname($file));
			}
		}
		...
		$stat['read']  = intval($this->attr($file, 'read', isset($stat['read']) ? !!$stat['read'] : false));
		$stat['write'] = intval($this->attr($file, 'write', isset($stat['write']) ? !!$stat['write'] : false));
		if ($root) {
			$stat['locked'] = 1;
		} elseif ($this->attr($file, 'locked', !empty($stat['locked']))) {
			$stat['locked'] = 1;
		} else {
			unset($stat['locked']);
		}
		...
		if ($root) {
			unset($stat['hidden']);
		} elseif ($this->attr($file, 'hidden', !empty($stat['hidden']))
		...
		if ($stat['read'] && empty($stat['hidden'])) {
			...
			if ($stat['mime'] == 'directory') {
				...
				$p = isset($stat['target']) ? $stat['target'] : $file;
	...
	protected function cacheDir($file) {
		...
		foreach ($this->_scandir($file) as $p) {
			if (($stat = $this->stat($p)) && empty($stat['hidden'])) {
				$this->dirsCache[$file][] = $p;
			}
		}	
	}
	...
	protected function mimetype($file) {
		...
		if ($this->mimeDetect == 'finfo') {
			$type = @finfo_file($this->finfo, $file);
		} elseif ($type == 'mime_content_type') {
			$type = mime_content_type($file);
		} else {
			$type = elFinderVolumeDriver::mimetypeInternalDetect($file);
		}
		...
		return $type == 'unknown' && $this->mimeDetect != 'internal'
			? elFinderVolumeDriver::mimetypeInternalDetect($file)
			: $type;
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	static protected function mimetypeInternalDetect($file) {
		$pinfo = pathinfo($file);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function countSize($file) {
		$stat = $this->stat($file);
		...
		foreach ($this->getScandir($file) as $stat) {
			$size = $stat['mime'] == 'directory' && $stat['read'] 
				? $this->countSize($this->_joinPath($file, $stat['name']))
				: $stat['size'];
			if ($size > 0) {
				$result += $size;
			}
	...
	/**
	 * If file has required attr == $val - return file file,
	 * If dir has child with has required attr == $val - return child file
	 *
	 * @param  string   $file  file file
	 * @param  string   $attr  attribute name
	 * @param  bool     $val   attribute value
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function closestByAttr($file, $attr, $val) {
		$stat = $this->stat($file);
		...
		return $stat['mime'] == 'directory'
			? $this->childsByAttr($file, $attr, $val)
			: false;
	}
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function childsByAttr($file, $attr, $val) {
		foreach ($this->_scandir($file) as $p) {
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function getScandir($file) {
		$files = array();
		
		!isset($this->dirsCache[$file]) && $this->cacheDir($file);

		foreach ($this->dirsCache[$file] as $p) {
			if (($stat = $this->stat($p)) && empty($stat['hidden'])) {
				$files[] = $stat;
			}
		}
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function gettree($file, $deep, $exclude='') {
		$dirs = array();
		...
		foreach ($this->dirsCache[$file] as $p) {
			...
			if ($stat && empty($stat['hidden']) && $file != $exclude && $stat['mime'] == 'directory') {
				$dirs[] = $stat;
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function doSearch($file, $q, $mimes) {
		$result = array();
	 ...
	 * @param  string  $dst   destination dir file
	 ...
	 * @param  string  $destination  destination dir file
	 * @param  string  $name         file name
	 * @return string|false
	 * @author Dmitry (dio) Levashov
	 **/
	protected function copyFrom($volume, $src, $destination, $name) {
		...
		if ($source['mime'] == 'directory') {
			...
			$file = $this->_joinPath($destination, $name);
			
			foreach ($volume->scandir($src) as $entr) {
				if (!$this->copyFrom($volume, $entr['hash'], $file, $entr['name'])) {
					return false;
				}
			}
			...
			|| ($file = $this->_save($fp, $destination, $name, $mime, $w, $h)) == false) {
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function remove($file, $force = false) {
		$stat = $this->stat($file);
		$stat['realpath'] = $file;
		if (!empty($stat['tmb']) && $stat['tmb'] != "1") {
			$this->rmTmb($stat['tmb']);
		...
		if (empty($stat)) {
			return $this->setError(elFinder::ERROR_RM, $this->_path($file), elFinder::ERROR_FILE_NOT_FOUND);
		}
		
		if (!$force && !empty($stat['locked'])) {
			return $this->setError(elFinder::ERROR_LOCKED, $this->_path($file));
		}
		...
		if ($stat['mime'] == 'directory') {
			...
			if (!$this->_rmdir($file)) {
				return $this->setError(elFinder::ERROR_RM, $this->_path($file));
			}
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function gettmb($file, $stat) {
		if ($this->tmbURL && $this->tmbPath) {
			// file itself thumnbnail
			if (strpos($file, $this->tmbPath) === 0) {
	 ...
	 *
	 * @param  string  $file  thumnbnail file
	 * @param  array   $stat  file stat
	 * @return string|bool
	 * @author Dmitry (dio) Levashov
	 **/
	protected function canCreateTmb($file, $stat) {
		return $this->tmbPathWritable 
			&& strpos($file, $this->tmbPath) === false // do not create thumnbnail for thumnbnail
			&& $this->imgLib 
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function canResize($file, $stat) {
		return $this->canCreateTmb($file, $stat);
	}
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	protected function createTmb($file, $stat) {
		if (!$stat || !$this->canCreateTmb($file, $stat)) {
			return false;
		}
		...
		if (($src = $this->_fopen($file, 'rb')) == false) {
			return false;
		}

		if (($trg = fopen($tmb, 'wb')) == false) {
			$this->_fclose($src, $file);
			return false;
		}
	 ...
	 * @author Alexey Sukhotin
	 **/
  	protected function imgResize($file, $width, $height, $keepProportions = false, $resizeByBiggerSide = true, $destformat = null) {
			...
			case 'gd':
				...
				if ($img &&  false != ($tmp = imagecreatetruecolor($size_w, $size_h))) {
					...
					if ($destformat == 'jpg'  || ($destformat == null && $s['mime'] == 'image/jpeg')) {
						$result = imagejpeg($tmp, $file, 100);
					} else if ($destformat == 'gif' || ($destformat == null && $s['mime'] == 'image/gif')) {
						$result = imagegif($tmp, $file, 7);
					} else {
						$result = imagepng($tmp, $file, 7);
					}
	 ...
	 * @author Alexey Sukhotin
	 **/
  	protected function imgCrop($file, $width, $height, $x, $y, $destformat = null) {
		if (($s = @getimagesize($file)) == false) {
			return false;
		}
	 ...
	 * @author Alexey Sukhotin
	 **/
		protected function imgSquareFit($file, $width, $height, $align = 'center', $valign = 'middle', $bgcolor = '#0000ff', $destformat = null) {
		if (($s = @getimagesize($file)) == false) {
			return false;
		}
	 ...
	 * @author Troex Nevelin
	 **/
	protected function imgRotate($file, $degree, $bgcolor = '#ffffff', $destformat = null) {
		if (($s = @getimagesize($file)) == false) {
			return false;
		}
	...
	**/
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _dirname($file);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _basename($file);

	/**
	 * Join dir name and file name and return full file.
	 * Some drivers (db) use int as file - so we give to concat file to driver itself
	 *
	 * @param  string  $dir   dir file
	 * @param  string  $name  file name
	 * @return string
	 * @author Dmitry (dio) Levashov
	 **/
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _normpath($file);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _relpath($file);
	
	/**
	 * Convert file related to root dir into real file
	 *
	 * @param  string  $file  rel file file
	 * @return string
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _abspath($file);
	
	/**
	 * Return fake file started from root dir.
	 * Required to show file on client side.
	 *
	 * @param  string  $file  file file
	 * @return string
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _path($file);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _inpath($file, $parent);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _stat($file);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _subdirs($file);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _dimensions($file, $mime);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _scandir($file);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _fopen($file, $mode="rb");
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _fclose($fp, $file='');
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _mkdir($file, $name);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _mkfile($file, $name);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _unlink($file);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _rmdir($file);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _getContents($file);
	 ...
	 * @author Dmitry (dio) Levashov
	 **/
	abstract protected function _filePutContents($file, $content);
	 ...
	 * @author Alexey Sukhotin
	 **/
	abstract protected function _extract($file, $arc);
