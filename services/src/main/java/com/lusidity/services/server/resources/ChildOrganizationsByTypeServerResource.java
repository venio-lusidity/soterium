...
@AtWebResource(pathTemplate = "/svc/domains/{domain}/{webId}/organizations_organization", methods = "get", description = "Get children organizations for the specified parents.", requiredParams = "url")
@AtAuthorization()
public class ChildOrganizationsByTypeServerResource
    ...
    public Representation retrieve() {
        ...
        try {
            ...
            if(!StringX.isBlank(webId) && !StringX.isBlank(type)) {
                ...
                if ((null!=vertex) && ClassX.isKindOf(vertex.getClass(), Organization.class)) {
                    Organization parent = (Organization) vertex;
                    List<Organization> children = new ArrayList<>();
                    Text t = new Text(type);
                    for (Organization child : parent.getOrganizations()) {
                        try {
                    ...
                    Collections.sort(children, new Comparator<Organization>() {
                        @Override
